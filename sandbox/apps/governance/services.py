...
ROLE_PERMISSIONS = {
    ...
    RoleAssignment.Role.RULE_CONFIGURER: {
        "alarm.view", "report.view", "report.generate", "report.publish",
        "export.masked", "rule.draft", "rule.submit",
    },
    ...
    RoleAssignment.Role.SYSTEM_ADMIN: {
        "identity.manage", "enterprise.manage", "system.configure", "rule.runtime",
...
def resolve_enterprise_for_user(user, company_id, company_name):
    ...
    for scope, _sensitive, _inherited_from in visible_enterprise_scopes(user):
...
def claim_shift(*, user, platform_account_ref, workstation_id, request_id=""):
    ...
    roles = active_roles(user)
    if RoleAssignment.Role.UNIT_USER not in roles:
        raise GovernanceError("只有单位使用人员可以认领班次", "SHIFT_ROLE_REQUIRED", 403)
