...
class FiveSourceReportTaskTests(TestCase):
    def setUp(self):
        ...
        assign_role(user=self.user, role=RoleAssignment.Role.UNIT_USER, assigned_by=self.user)
        ...
        assign_role(user=self.collector, role=RoleAssignment.Role.UNIT_USER, assigned_by=self.user)
