(() => {
  ...
  function renderEvents() {
    ...
    const counts = {
      REALTIME: items.filter((item) => item.event.sourceKind === "REALTIME").length,
      TECHNICAL: items.filter((item) => item.event.sourceKind === "TECHNICAL").length,
      PENDING: items.filter((item) => item.event.sourceKind === "PENDING").length,
    ...
    const prewarningFreshness = Number.isFinite(realtimePollAge)
      ...
      : "尚未在实时监控预警列表或预警查询页捕获";
    const priorityKinds = ["REALTIME", "TECHNICAL", "PENDING"];
    const visibleItems = alarmView === "priority"
      ? items.filter((item) => priorityKinds.includes(item.event.sourceKind))
      : alarmView === "all" ? items : items.filter((item) => item.event.sourceKind === alarmView);
    target.innerHTML = `<div class="source-tabs">
      <button class="source-tab ${alarmView === "priority" ? "active" : ""}" data-source="priority">优先队列 ${counts.REALTIME + counts.TECHNICAL + counts.PENDING}</button>
      <button class="source-tab ${alarmView === "REALTIME" ? "active" : ""}" data-source="REALTIME">实时报警 ${counts.REALTIME}</button>
      <button class="source-tab ${alarmView === "TECHNICAL" ? "active" : ""}" data-source="TECHNICAL">技术检测 ${counts.TECHNICAL}</button>
      <button class="source-tab ${alarmView === "PENDING" ? "active" : ""}" data-source="PENDING">待处理正式报警 ${counts.PENDING}</button>
