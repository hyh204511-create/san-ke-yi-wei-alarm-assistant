...
> 当前源码版本：`0.5.6`。真实联调已确认实时监控页 `#/vehicle-monitor/real-time` 中 `getVideoUnprocessedAlarm` 对应“预警列表”；同页内部的“实时报警”页签请求 `alarmQueryList` 和 SharedWorker 的正式 `ALARM_ADD` 消息归类为正式实时报警，“预警列表”页签和 `alarmKind=0` 消息才归类为 `PREWARNING`。插件优先监听页面主动请求和已存在的只读长连接消息，只有监听不到且页面已有成功查询时才按3秒周期兜底采集，无需刷新整页。平台数据到达后目标是在1秒内进入插件展示。自动动作由已发布规则选择 `TEXT_TTS` 或 `VOICE_INTERCOM`；真实语音/文本适配器仍需在测试车辆上完成授权联调。
