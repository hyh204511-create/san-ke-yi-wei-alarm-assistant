...
> 当前源码版本：`0.5.7`。真实页面观测记录（2026-07-22）：实时监控页 `#/vehicle-monitor/real-time` 中平台“实时报警”显示 1、技术检测显示 0、预警列表显示 63，插件面板同步显示正式报警 1、技术检测 0、预警列表 63；`getVideoUnprocessedAlarm` 对应“预警列表”，同页“实时报警”页签请求 `alarmQueryList` 和 SharedWorker 的正式 `ALARM_ADD` 消息归类为正式报警。页面在请求发起时尚未渲染活动页签时，插件会在响应完成时按原请求路由重新判断，避免误落到历史查询。插件优先监听页面主动请求和已存在的只读长连接消息，只有监听不到且页面已有成功查询时才按3秒周期兜底采集，无需刷新整页。自动动作由已发布规则选择 `TEXT_TTS` 或 `VOICE_INTERCOM`；真实语音/文本适配器仍需在测试车辆上完成授权联调。
