...
def admin_action_page(request, operation):
    ...
    try:
        ...
        elif operation == "platform-enterprise-discovery":
            enterprise = EnterpriseScope.objects.filter(public_id=uuid_value(request.POST.get("enterprise_id"), "enterprise_id"), is_active=True).first()
            if not enterprise: raise services.GovernanceError("企业范围不存在", "ENTERPRISE_NOT_FOUND", 404)
            services.update_platform_enterprise_discovery(
                actor=request.user, enterprise=enterprise,
                enabled=request.POST.get("enabled") == "on", request_id=getattr(request, "request_id", ""),
            )
...
def voice_interaction_policy_api(request):
    if request.method == "GET":
        permissions = services.permissions_for_roles(services.active_roles(request.user))
        if not {"action.execute", "system.configure", "session.keepalive.execute"}.intersection(permissions):
