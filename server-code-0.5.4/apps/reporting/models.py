...
class ReportSnapshot(TimeStampedModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["enterprise", "period_type", "period_start", "version"], name="unique_report_snapshot_version"),
            models.UniqueConstraint(fields=["enterprise", "period_type", "period_start"], condition=Q(status="PUBLISHED"), name="unique_published_report_period"),
        ]
        indexes = [models.Index(fields=["enterprise", "period_type", "-period_start"], name="report_scope_period_idx")]
...
class ExportJob(TimeStampedModel):
    ...
    public_id = models.UUIDField(default=uuid.uuid4, unique=True, editable=False)
    report_snapshot = models.ForeignKey(ReportSnapshot, on_delete=models.PROTECT, related_name="export_jobs")
